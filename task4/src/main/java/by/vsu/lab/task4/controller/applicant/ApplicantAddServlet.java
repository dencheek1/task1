...
public class ApplicantAddServlet extends BaseServlet {
	...
	if (id != null) {
		...
		logger.debug("applicant id id={}",id);
		if(factory.getApplicantService().findById(id) != null) {
			resp.sendRedirect(req.getContextPath()
				+ "/applicant.html?applicant_id=" + applicant.getId()
				+ "&message= applicant alredy registred");
			return;
		}
		...
		getServletContext().getRequestDispatcher("/WEB-INF/jsp/applicant/edit.jsp").forward(req, resp);
